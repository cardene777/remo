...
class WorkoutBanner(GridLayout):
    def __init__(self, **kwargs):
        ...
        left = FloatLayout()
        left_image = Image(source=f"icons/workouts/{kwargs['workout_image']}", size_hint=(1, 0.8),
        ...
        middle = FloatLayout()
        middle_image = Image(source=f"icons/{kwargs['type_image']}", size_hint=(1, 0.8),
                             pos_hint={"top": 1, "right": 1})
        middle_label = Label(text=f"{str(kwargs['number'])} first bumps",
                             size_hint=(1, .2), pos_hint={"top": .2, "right": 1})
        ...
        right_label = Label(text=f"{str(kwargs['likes'])} {kwargs['units']}", size_hint=(1, .2),
